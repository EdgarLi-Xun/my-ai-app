...
async function onNewConversation() {
  ...
  try {
    ...
    uni.navigateTo({ url: `/pages/conversation-detail/conversation-detail?id=${c.id}` });
  } catch (err: unknown) {
    const e = err as { message?: string };
...
<template>
  <view class="page">
    ...
    <scroll-view class="scroll" scroll-y>
      ...
      <view v-for="c in conversations" :key="c.id" class="item" @click="openConversation(c)">
      ...
      <view v-if="deletedConversations.length > 0" class="deleted-section">
        ...
        <view v-if="showDeleted">
          ...
          >
            <text class="item-title">{{ c.title || '新对话' }}</text>
            <text class="item-time">点击恢复</text>
          </view>
        </view>
      </view>
    </scroll-view>
